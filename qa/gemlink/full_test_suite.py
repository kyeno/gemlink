#!/usr/bin/env python2
#
# Execute all of the automated tests related to Gemlink.
#
...
def test_rpath_runpath(filename):
    output = subprocess.check_output(
        [repofile('qa/gemlink/checksec.sh'), '--file', repofile(filename)]
    )
...
def test_fortify_source(filename):
    proc = subprocess.Popen(
        [repofile('qa/gemlink/checksec.sh'), '--fortify-file', repofile(filename)],
...
def check_security_hardening():
    ...
    ret &= test_rpath_runpath('src/gemlinkd')
    ret &= test_rpath_runpath('src/gemlink-cli')
    ret &= test_rpath_runpath('src/gemlink-gtest')
    ret &= test_rpath_runpath('src/gemlink-tx')
    ret &= test_rpath_runpath('src/test/test_bitcoin')
    ret &= test_rpath_runpath('src/gemlink/GenerateParams')

    # NOTE: checksec.sh does not reliably determine whether FORTIFY_SOURCE
    # is enabled for the entire binary. See issue #915.
    ret &= test_fortify_source('src/gemlinkd')
    ret &= test_fortify_source('src/gemlink-cli')
    ret &= test_fortify_source('src/gemlink-gtest')
    ret &= test_fortify_source('src/gemlink-tx')
    ret &= test_fortify_source('src/test/test_bitcoin')
    ret &= test_fortify_source('src/gemlink/GenerateParams')
...
STAGE_COMMANDS = {
    ...
    'gtest': [repofile('src/gemlink-gtest')],
