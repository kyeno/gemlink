...
CBudgetProposal::CBudgetProposal() : nAlloted(0),
                                     fValid(true),
                                     strInvalid(""),
...
CBudgetProposal::CBudgetProposal(const std::string& name,
                                 ...
                                 int blockstart,
                                 const uint256& nfeetxhash) : nAlloted(0),
                                                              fValid(true),
                                                              strInvalid(""),
...
bool CBudgetProposal::CheckStartEnd()
{
    ...
    bool fNewRules = Params().GetConsensus().NetworkUpgradeActive(nBlockStart, Consensus::UPGRADE_MORAG);
...
bool CBudgetProposal::UpdateValid(int nCurrentHeight, bool fCheckCollateral)
{
    ...
    bool fNewRules = Params().GetConsensus().NetworkUpgradeActive(nCurrentHeight, Consensus::UPGRADE_MORAG);

    // Never kill a proposal before the first superblock
    if (!fNewRules || nCurrentHeight > nBlockStart) {
        if (IsHeavilyDownvoted(fNewRules))
            return false;
    }
