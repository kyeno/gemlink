...
std::vector<CSporkDef> sporkDefs = {
    ...
    MAKE_SPORK_DEF(SPORK_19_MASTERNODE_PAYMENT_ENFORCEMENT_MORAG, 1646067600ULL), // ON
};
...
void ReprocessBlocks(const CChainParams& chainparams, int nBlocks)
{
    ...
    while (it != mapRejectedBlocks.end()) {
        ...
        if ((*it).second > GetTime() - (nBlocks * chainparams.GetConsensus().nPowTargetSpacing * 2)) {
