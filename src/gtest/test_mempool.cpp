...
TEST(Mempool, TxInputLimit) {
    ...
    EXPECT_FALSE(AcceptToMemoryPool(Params(), pool, state2, tx1, false, &missingInputs));
    ...
    EXPECT_FALSE(AcceptToMemoryPool(Params(), pool, state3, tx3, false, &missingInputs));
    ...
    EXPECT_FALSE(AcceptToMemoryPool(Params(), pool, state4, tx3, false, &missingInputs));
    ...
    EXPECT_FALSE(AcceptToMemoryPool(Params(), pool, state5, tx3, false, &missingInputs));
    // The -mempooltxinputlimit check doesn't set a reason
    ...
    EXPECT_FALSE(AcceptToMemoryPool(Params(), pool, state6, tx3, false, &missingInputs));
