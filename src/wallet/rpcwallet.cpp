...
static void SendMoneyNoChange(const CTxDestination& address, CAmount nValue, const CTxDestination& targetChange, bool fSubtractFeeFromAmount, CWalletTx& wtxNew)
{
    CAmount curBalance = pwalletMain->GetBalance();
    ...
    coinControl.destChange = targetChange;
...
UniValue sendtoaddressnochange(const UniValue& params, bool fHelp)
{
    if (!EnsureWalletIsAvailable(fHelp))
        return NullUniValue;
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            ...
            CURRENCY_UNIT + " to send. eg 0.1\n"
                            ...
                            "\"transactionid\"  (string) The transaction id.\n"
                            "\nExamples:\n" +
            HelpExampleCli("sendtoaddressnochange", "\"t1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1") + HelpExampleCli("sendtoaddressnochange", "\"t1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1 \"donation\" \"seans outpost\"") + HelpExampleCli("sendtoaddressnochange", "\"t1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1 \"\" \"\" true") + HelpExampleRpc("sendtoaddressnochange", "\"t1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\", 0.1, \"donation\", \"seans outpost\""));
    ...
    auto firstAddr = vKeyBirth.begin();
    const CKeyID& keyid = firstAddr->second;
    std::string strAddr = keyIO.EncodeDestination(keyid);

    CTxDestination destChange = keyIO.DecodeDestination(strAddr);

    LogPrintf("Dest for change %s\n", keyIO.EncodeDestination(destChange));
    SendMoneyNoChange(dest, nAmount, destChange, fSubtractFeeFromAmount, wtx);
...
UniValue sendmanynochange(const UniValue& params, bool fHelp)
{
    if (!EnsureWalletIsAvailable(fHelp))
        return NullUniValue;
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendmanynochange \"fromaccount\" {\"address\":amount,...} ( minconf \"comment\" [\"address\",...] )\n"
            ...
            HelpExampleCli("sendmanynochange", "\"\" \"{\\\"t14oHp2v54vfmdgQ3v3SNuQga8JKHTNi2a1\\\":0.01,\\\"t1353tsE8YMTA4EuV7dgUXGjNFf9KpVvKHz\\\":0.02}\"") +
            "\nSend two amounts to two different addresses setting the confirmation and comment:\n" + HelpExampleCli("sendmanynochange", "\"\" \"{\\\"t14oHp2v54vfmdgQ3v3SNuQga8JKHTNi2a1\\\":0.01,\\\"t1353tsE8YMTA4EuV7dgUXGjNFf9KpVvKHz\\\":0.02}\" 6 \"testing\"") +
            "\nSend two amounts to two different addresses, subtract fee from amount:\n" + HelpExampleCli("sendmanynochange", "\"\" \"{\\\"t14oHp2v54vfmdgQ3v3SNuQga8JKHTNi2a1\\\":0.01,\\\"t1353tsE8YMTA4EuV7dgUXGjNFf9KpVvKHz\\\":0.02}\" 1 \"\" \"[\\\"t14oHp2v54vfmdgQ3v3SNuQga8JKHTNi2a1\\\",\\\"t1353tsE8YMTA4EuV7dgUXGjNFf9KpVvKHz\\\"]\"") +
            "\nAs a json rpc call\n" + HelpExampleRpc("sendmanynochange", "\"\", \"{\\\"t14oHp2v54vfmdgQ3v3SNuQga8JKHTNi2a1\\\":0.01,\\\"t1353tsE8YMTA4EuV7dgUXGjNFf9KpVvKHz\\\":0.02}\", 6, \"testing\""));
...
UniValue ListReceived(const UniValue& params, bool fByAccounts)
{
    ...
    for (const std::pair<CTxDestination, CAddressBookData>& item : pwalletMain->mapAddressBook) {
        ...
        if (fByAccounts) {
            ...
            obj.push_back(Pair("address", keyIO.EncodeDestination(dest)));
...
static void MaybePushAddress(UniValue& entry, const CTxDestination& dest)
{
    if (IsValidDestination(dest)) {
        KeyIO keyIO(Params());
        entry.push_back(Pair("address", keyIO.EncodeDestination(dest)));
    }
}
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    BOOST_FOREACH (const COutput& out, vecOutputs) {
        ...
        if (fValidAddress) {
            entry.push_back(Pair("address", keyIO.EncodeDestination(address)));
...
UniValue z_gettotalbalance(const UniValue& params, bool fHelp)
{
    ...
    CAmount nPrivateBalance = getBalanceZaddr(std::nullopt, nMinDepth, !fIncludeWatchonly);
...
static const CRPCCommand commands[] =
    { //  category              name                        actor (function)           okSafeMode
      //  --------------------- ------------------------    -----------------------    ----------
        ...
        {"wallet", "sendmanynochange", &sendmanynochange, false},
        {"wallet", "sendtoaddress", &sendtoaddress, false},
        {"wallet", "sendtoaddressnochange", &sendtoaddressnochange, false},
