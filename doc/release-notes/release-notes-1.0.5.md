...
Instruct users to run gemlink-fetch-params if network params aren't available
...
Throw an error if gemlink.conf is missing
Show a friendly message explaining why gemlinkd needs a gemlink.conf
Fix gtest ordering broken by #1949
Debian package lint
Generate Debian control file to fix shlibs lint
Create empty gemlink.conf during performance measurements
Create empty gemlink.conf during coverage checks
...
Remove UTF-8 BOM efbbbf from gemlink.conf to avoid problems with command line tools
Closes #1097 so gemlink-cli now displays license info like gemlinkd.
