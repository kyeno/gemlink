...
This is an experimental feature. Enable it by launching `gemlinkd` with flags:

    gemlinkd -experimentalfeatures -paymentdisclosure -debug=paymentdisclosure -txindex=1

These flags can also be set as options in `gemlink.conf`.
...
[1] https://github.com/gemlink/zips/pull/119

### Example Use Case

Alice the customer sends 10 GLINK to Bob the merchant at the shielded address shown on their website. However, Bob is not sure if he received the funds.
...
Generate a payment disclosure for the first joinsplit, second output (index starts from zero):
    ...
    gemlink-cli z_getpaymentdisclosure 79189528d611e811a1c7bb0358dd31343033d14b4c1e998d7c4799c40f8b652b 0 1 "Hello"
...
To validate the payment disclosure:
    ...
    gemlink-cli z_validatepaymentdisclosure HEXDATA
...
For nodes that generate payment disclosures, a LevelDB database is created in the node's datadir. For most users, this would be in the folder:
    ...
    $HOME/.gemlink/paymentdisclosure
